...
export default function ConnectionFinderPage() {
  ...
  if (connectionBridgeResult) {
    ...
    return (
      <div className="flex-1 flex flex-col md:flex-row overflow-hidden bg-[var(--bg)] animate-fade-in">
        ...
        <aside className="w-full md:w-[320px] lg:w-[350px] border-b md:border-b-0 md:border-r border-[var(--border)] flex flex-col p-5 sm:p-8 overflow-y-auto shrink-0 bg-[var(--bg-sidebar)] max-h-[260px] md:max-h-none">

          <button
            onClick={handleResetSearch}
            className="text-[11px] text-[var(--muted)] hover:text-[var(--text)] mb-5 sm:mb-8 tracking-[0.1em] uppercase text-left cursor-pointer transition-colors"
          >
            ← New Bridge Search
          </button>
          ...
          <button
            ...
            className="mt-6 sm:mt-12 w-full text-center text-[12px] tracking-[0.12em] uppercase text-[var(--text)] border border-[var(--border)] py-2.5 sm:py-3 hover:border-[var(--text)] hover:bg-[var(--bg-panel)] transition-all cursor-pointer font-medium"
          >
        ...
        <main className="flex-1 overflow-y-auto p-6 sm:p-10 md:p-14 lg:p-16 flex flex-col gap-8 sm:gap-12 max-w-4xl">
          ...
          <section className="flex flex-col gap-3 sm:gap-4">
            ...
            <p
              className="text-lg sm:text-xl md:text-2xl text-[var(--text)] leading-relaxed font-serif tracking-tight"
          ...
          {activeStep && (
            <section className="flex flex-col gap-6 sm:gap-8 animate-fade-in">
              <div className="flex flex-col sm:flex-row gap-5 sm:gap-8 items-start">

                {activeStep.image && (
                  <div className="w-full sm:w-[220px] h-[140px] sm:h-[160px] relative overflow-hidden bg-[var(--bg-panel)] border border-[var(--border)] shrink-0">
                    <img
  ...
  return (
    <div className="flex-1 flex flex-col justify-center px-6 sm:px-12 md:px-20 max-w-4xl animate-fade-up bg-[var(--bg)] py-8 sm:py-0">

      <h1
        className="font-serif text-4xl sm:text-6xl md:text-[72px] leading-[0.94] tracking-tight text-[var(--text)] mb-6 sm:mb-10"
        style={{ fontFamily: 'var(--font-dm-serif)' }}
      >
      ...
      <p className="text-xs sm:text-[13px] text-[var(--muted)] leading-relaxed max-w-xl mb-6 sm:mb-10">
      ...
      <form onSubmit={handleConnect} className="flex flex-col gap-6 sm:gap-8 max-w-xl">
        ...
        <div className="flex flex-col gap-1.5">
          ...
          <input
            ...
            className="bg-transparent text-[var(--text)] text-sm sm:text-base placeholder-[var(--muted)] border-b border-[var(--border)] focus:border-[var(--text)] pb-2 focus:outline-none transition-colors duration-200 disabled:opacity-50"
          />
        </div>
        ...
        <div className="flex items-center gap-4">
          <button
            ...
            className="text-[12px] tracking-[0.12em] uppercase text-[var(--text)] border border-[var(--border)] px-6 py-2.5 hover:border-[var(--text)] hover:bg-[var(--bg-panel)] transition-all duration-150 disabled:opacity-30 disabled:cursor-not-allowed cursor-pointer w-full sm:w-auto text-center"
          >
      ...
      <div className="mt-6 sm:mt-8 flex flex-wrap items-center gap-x-2.5 sm:gap-x-3 gap-y-2 animate-fade-in max-w-xl">
        ...
        {RECOMMENDATIONS.map((r, i) => (
          <React.Fragment key={i}>
            <button
              ...
              disabled={localLoading}
              className="text-[11px] text-[var(--muted)] hover:text-[var(--text)] transition-colors cursor-pointer font-mono disabled:opacity-40 py-0.5"
            >
              {r.from} &rarr; {r.to}
            </button>
